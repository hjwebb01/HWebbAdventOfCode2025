...
function adventDay2(range) {
    var bounds = range.split("-").map(Number); // Split the range into lower and upper bounds
    var badID = 0;
    for (var i = bounds[0]; i <= bounds[1]; i++) {
        // Loop through each number in the range
        var numStr = i.toString();
        if (numStr.length % 2 != 0)
            continue; // Skip if the number of digits is odd
        var l = 0;
...
function main() {
    var input = (0, fs_1.readFileSync)("./input.txt", "utf8");
    var res = 0;
